...
resource "aws_lb_target_group" "target_group" {
  ...
  port     = var.instance_port[0]      ##### Don't use protocol when target type is lambda
  protocol = var.instance_protocol[0]  ##### Don't use protocol when target type is lambda
  vpc_id   = var.vpc_id
  target_type = var.target_type_alb[0]
  stickiness {
  type = var.stickiness_type[1] ## The type of sticky sessions. The only current possible values are lb_cookie for ALBs and source_ip for NLBs.
  ...
  health_check {
    ...
    protocol = var.health_check_protocol[0]
